...
void loadElectionInfoFromFile()
{
    ...
    printf(" The given year was : %d\n", currentValidID.year);
    fseek(f1, 2, SEEK_CUR);
    fscanf(f1, "%s", currentValidID.branch);
    printf(" The given branch was : %s\n", currentValidID.branch);
    fseek(f1, 2, SEEK_CUR);
    fscanf(f1, "%d", &currentValidID.totalVoters);
    printf(" The given number of voters was : %d\n", currentValidID.totalVoters);
    fseek(f1, 2, SEEK_CUR);
    fscanf(f1, "%d", &numberOfCandidates);
    printf(" The given number of candidates was : %d\n", numberOfCandidates);
    fclose(f1);
    FILE *candidateFile;
    char filename[20];
    char dis;
    ...
    {
        sprintf(filename, "candidate%d.txt", i);
        candidateFile = fopen(filename, "r+");
        if (candidateFile == NULL)
            printf(" No file found...\n");

        fscanf(candidateFile, "%s", candidateArray[i - 1].cname);
        printf(" Name: %s\n", candidateArray[i - 1].cname);
        fseek(candidateFile, 2, SEEK_CUR);
        fscanf(candidateFile, "%d", &candidateArray[i - 1].votes);
...
void banID()
{
    printf("\nCreating Banned File....\n");
    FILE *fp = fopen("Banned.txt", "r+");
    if (fp = NULL)
    {
        printf("\nError!\n");
        fclose(fp);
    ...
    while (1)
    {
        printf("\nEnter the last three digits of your USN\n Press 0 to exit\n");
        scanf("%d", &input);
        if (input == 0)
        {
            break;
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            case '2':
                ...
                break;
            case '3':
                printf("Enter user ID you want to delete its vote: ");
                scanf("%s", inputID);
